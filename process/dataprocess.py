...
class processer():
    ...
    def create_examples(self,data,type):
        ...
        for i,line in enumerate(data):
            guid = f'{i}-{line}'
            text_a = line.split('\t')[1].replace('\t','').replace('\n','')
            text_b = None
    ...
    def create_dataset(self,features):
        ...
        dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_lens, all_labels)#几个参数以元组的形式传入到TensorDataset中，在TensorDataset中依次取出每个样本的input_ids等
        return dataset
